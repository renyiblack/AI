...
from loot import *
from heal import HealLowLVL


def HuntLow(starter_mark, max_markers):
    ...
    while on == 0:
        ...
        try:
            if keyboard.is_pressed('q'):
                ...
                heal_pos = imagesearcharea(heal, 443, 568, 476, 619)  # search for heal TODO change to a pixel
                if not heal_pos[0] == -1:
                    HealLowLVL()
                # look at battle list and see if first target is red or pink, if so, you're attacking
                if (pyautogui.pixelMatchesColor(1194, 456, (255, 0, 0)) or 
                	pyautogui.pixelMatchesColor(1194, 456, (255, 128,128))):
